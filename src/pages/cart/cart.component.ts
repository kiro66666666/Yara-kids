      ...
      <div class="container mx-auto px-6 max-w-6xl">
        ...
        @if (store.cart().length > 0) {
          <div class="flex flex-col lg:flex-row gap-10 items-start">
            <div class="flex-1 space-y-4 w-full">
               @for (item of store.cart(); track item.id + item.selectedSize) {
                 <div class="flex flex-col sm:flex-row gap-6 p-6 bg-white dark:bg-brand-darksurface rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all items-center">
                   <div class="w-full sm:w-28 aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-gray-800 flex-shrink-0 border border-gray-200 dark:border-gray-700">
                   ...
                   <div class="flex-1 w-full text-center sm:text-left">
                     <div class="flex flex-col sm:flex-row justify-between items-start mb-2">
                        ...
                        <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Ref: {{ item.id }}</div>
                     </div>
                     
                     <p class="text-sm text-gray-600 dark:text-gray-400 mb-4 flex items-center justify-center sm:justify-start gap-2">
                       <span class="w-4 h-4 rounded-full border border-gray-300 dark:border-gray-600 shadow-sm" [style.background-color]="item.colors[0].hex"></span>
                     ...
                     <div class="flex items-center justify-between mt-4 sm:mt-auto">
                        ...
                        <div class="flex items-center gap-4">
                          <div class="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-xl px-3 py-1.5 border border-gray-300 dark:border-gray-700">
                            <button
                              (click)="store.updateQuantity(item.id, item.selectedSize, item.selectedColor, -1)"
                              class="w-8 h-8 flex items-center justify-center font-bold text-gray-500 dark:text-gray-400 hover:text-brand-pink hover:bg-white dark:hover:bg-gray-700 rounded-lg transition-all text-lg leading-none pb-1"
                            >
                              -
                            </button>
                          ...
                          <button (click)="store.removeFromCart(item.id, item.selectedSize, item.selectedColor)" class="w-10 h-10 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all" title="Remover">
            ...
            <div class="w-full lg:w-[380px] flex-shrink-0 sticky top-24">
              <div class="bg-white dark:bg-brand-darksurface p-8 rounded-3xl shadow-card border border-gray-200 dark:border-gray-700">
                 ...
                 <div class="space-y-4 mb-6 text-sm">
                   ...
                   @if (store.discountAmount() > 0) {
                     <div class="flex justify-between text-green-700 dark:text-green-400 font-bold bg-green-50 dark:bg-green-900/20 p-2 rounded-lg -mx-2">
                   ...
                   <div class="flex justify-between text-gray-700 dark:text-gray-400">
                     <span>Frete</span>
                     <span class="text-xs text-gray-500 dark:text-gray-400 italic">Calcular no checkout</span>
                   </div>
                 </div>
                 ...
                 <div class="mb-6 relative">
                    ...
                    @if (showCoupons) {
                      <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 space-y-2 mb-4 animate-fade-in relative z-10 shadow-lg">
                         <div class="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400 mb-2 pl-1">Toque para aplicar</div>
                         @for(coupon of store.coupons(); track coupon.id) {
                           <div (click)="applySpecificCoupon(coupon.code)" class="bg-white dark:bg-brand-darksurface p-3 rounded-lg border border-dashed border-gray-300 dark:border-gray-600 cursor-pointer hover:border-brand-pink hover:bg-pink-50 dark:hover:bg-brand-pink/20 transition-colors group flex justify-between items-center shadow-sm">
                              <div>
                                ...
                                <span class="text-[10px] text-gray-600 dark:text-gray-400">{{ coupon.description }}</span>
                              </div>
                         ...
                         @if(store.coupons().length === 0) {
                           <p class="text-xs text-gray-500 dark:text-gray-400 italic text-center py-2">Nenhum cupom ativo no momento.</p>
                         }
                      </div>
                    }
                    ...
                    <div class="flex flex-col gap-2">
                      <div class="flex gap-2">
                        <input type="text" [(ngModel)]="couponCode" placeholder="Digitar código manualmente" class="flex-1 surface-input-light dark:bg-gray-800 dark:border-gray-700 rounded-xl px-4 py-3 text-sm font-bold focus:border-brand-pink focus:outline-none uppercase dark:text-white">
                    ...
                    @if (store.appliedCoupon()) {
                      <div class="flex items-center justify-between bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/30 p-3 rounded-xl mt-3 animate-fade-in">
                        ...
                        <button (click)="store.removeCoupon()" class="text-xs text-red-500 hover:text-red-600 font-bold underline">Remover</button>
                      </div>
                    }
                 </div>

                 <div class="border-t border-dashed border-gray-300 dark:border-gray-700 my-6"></div>

                 <div class="flex justify-between items-end mb-8">
                   <span class="text-gray-700 dark:text-gray-400 font-medium">Total Estimado</span>
                 ...
                 <div class="space-y-3">
                   ...
                   <a routerLink="/catalogo" class="block w-full py-4 bg-white dark:bg-transparent border-2 border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-400 font-bold text-center rounded-2xl hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-800 dark:hover:text-white transition-all">
                     Continuar Comprando
                   </a>
                 </div>

                 <div class="mt-6 flex items-center justify-center gap-2 text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 py-2 rounded-lg">
          ...
          <div class="flex flex-col items-center justify-center py-24 px-4 text-center">
             <div class="w-48 h-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full flex items-center justify-center mb-8 text-gray-500 dark:text-gray-300 shadow-inner relative">
             ...
             <p class="text-gray-600 dark:text-gray-400 mb-10 max-w-md text-lg">Parece que você ainda não escolheu os looks da sua princesa. Que tal dar uma olhadinha nas novidades?</p>
