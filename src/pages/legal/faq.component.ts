...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#f7f8fc] dark:bg-brand-darkbg py-12 transition-colors duration-300">
      <div class="container mx-auto px-4 max-w-3xl">
        ...
        <div class="space-y-4 animate-slide-up" style="animation-delay: 0.1s;">
          @for (item of filteredFaqs(); track item.id) {
            <details class="group bg-white dark:bg-brand-darksurface rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md hover:shadow-lg transition-shadow">
              <summary class="flex justify-between items-center p-6 cursor-pointer list-none font-bold text-gray-800 dark:text-white hover:text-brand-pink transition-colors select-none">
                ...
                <span class="transition-transform group-open:rotate-180 text-gray-500 dark:text-gray-400">
                  <app-icon name="chevron-down" size="20px"></app-icon>
                </span>
              </summary>
              <div class="px-6 pb-6 text-gray-700 dark:text-gray-300 text-sm leading-relaxed border-t border-gray-200 dark:border-gray-700 pt-4 whitespace-pre-wrap">
                {{ item.answer }}
              </div>
            </details>
          }
          @if (filteredFaqs().length === 0) {
             <div class="text-center py-10">
               <p class="text-gray-600 dark:text-gray-400">Nenhuma pergunta encontrada para sua busca.</p>
             </div>
          }
        </div>
