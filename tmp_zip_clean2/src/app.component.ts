...
@Component({
  ...
  styles: [`
    /* Global Reset for Images to prevent overflow */
    :host ::ng-deep img { max-width: 100%; height: auto; }
  `],
  template: `
    <!-- Offline Banner -->
    @if (!isOnline()) {
    ...
    <!-- Wrapper with updated padding-bottom (pb-24) to account for bottom nav + safe area -->
    <div class="flex flex-col min-h-[100dvh] font-sans text-gray-900 bg-white dark:bg-brand-darkbg dark:text-gray-100 pb-24 lg:pb-0 relative w-full max-w-full overflow-x-hidden transition-colors duration-300"
